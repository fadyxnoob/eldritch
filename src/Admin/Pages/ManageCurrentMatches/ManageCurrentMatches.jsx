...
const ManageCurrentMatches = () => {
    ...
    const getUpcomingMatches = useCallback(async () => {
        try {
            const response = await DatabaseService.getMatches('Pending');
            const thisMatch = await Promise.all(
                response.documents.map(async (match) => {
                    ...
                    if (match['player1']) {
                        try {
                            const player1Data = await DatabaseService.getDocument(match['player1']);
                    ...
                    if (match['player2']) {
                        try {
                            const player2Data = await DatabaseService.getDocument(match['player2']);
